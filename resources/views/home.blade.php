...
@section('content')
    <div class="bg-gray-600 px-20 py-16 rounded-lg mb-8 relative overflow-hidden">
        ...
        <p class="text-sm text-gray-400  mt-2">Proyecto de desarrolo web para profesionales.</p>
        <img src="{{asset('imagen/dev.png')}}" class="absolute -right-20 -bottom-20 opacity">
    </div>
    <div class="px-4">
            ...
            @foreach ($posts as $post)
                <a href="{{route('post',$post->slug)}}" class="bg-gray-100 rounded-lg px-6 py-4">
                    <p class="text-xs flex items-center gap-2">
                        <span class="uppercase bg-gray-300 rounded-full px-2 py-1">Tutorial</span>
                        <span> {{ $post->created_at->format('d/m/Y') }}</span>
                    </p>

                    <h2 class="text-lg text-gray-900 mt-2">{{ $post->title }}</h2>
                    <div class="text-xs text-gray-900 flex items-center gap-1">
